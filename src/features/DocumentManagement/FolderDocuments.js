...
export default function FolderDocuments({ folderId }) {
	...
	useEffect(() => {
		const fetchFolderData = async () => {
			try {
				...
				setFolder(
					Array.isArray(folderData) && folderData.length > 0
	...
	const onClickValider = async () => {
		...
		try {
			setLoading(true);
			const a = await fetch("/api/documents/valider", {
				method: "POST",
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(
					errorData.error ||
						errorData.message ||
						"Failed to import documents from Google Drive"
				);
			}
			...
			<div className="flex flex-grow overflow-hidden">
				...
				<div className="w-52  border-r border-gray-200 flex flex-col">
					...
					<nav className="flex-1 mt-6">
						<div className="flex items-center px-4 py-2  border-l-4 border-blue-600 text-blue-600">
							...
							<span>Documents</span>
							<span className="ml-auto bg-gray-200 rounded-full w-5 h-5 flex items-center justify-center text-xs">
								1
							</span>
						</div>

						<div
							onClick={onClickParsed}
							className="flex items-center px-4 py-2 hover:bg-[#FFFFFF40] cursor-pointer"
						>
							<svg
								xmlns="http://www.w3.org/2000/svg"
								className="h-5 w-5 mr-2"
				...
				<div className="flex-1 flex flex-col overflow-hidden">
					...
					<div className="p-4 border-t border-gray-200 flex justify-between">
						<button onClick={onClickValider} className="bg-blue-600 cursor-pointer text-white px-4 py-2 rounded-md hover:bg-blue-700">
							{loading?"...":"Traiter les documents"}
						</button>
						<button
					...
					<div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
						...
						<div className="mb-6">
							...
							<p className="mt-1 text-sm text-gray-500">
								Paste the URL of a Google Drive folder to import
								all files from it.
							</p>
						</div>
